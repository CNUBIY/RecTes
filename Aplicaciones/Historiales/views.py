...
def generate_bmi_chart_girls(request, idPat):
    try:
        ...
        # Guardar gráfico en el directorio de medios
        media_dir = settings.MEDIA_ROOT
        bmi_charts_dir = os.path.join(media_dir, 'bmi_charts_girls')
        if not os.path.exists(bmi_charts_dir):
            os.makedirs(bmi_charts_dir)

        # Eliminar la imagen anterior si existe
        image_path = os.path.join(bmi_charts_dir, f'bmi_chart_girls_{idPat}.png')
        if os.path.exists(image_path):
            os.remove(image_path)
...
def generate_height_chart_2_to_20(request, idPat):
    try:
        ...
        # Guardar gráfico en el directorio de medios
        media_dir = settings.MEDIA_ROOT
        height_charts_2_20_dir = os.path.join(media_dir, 'height_charts_2_to_20')
        if not os.path.exists(height_charts_2_20_dir):
            os.makedirs(height_charts_2_20_dir)

        # Eliminar la imagen anterior si existe
        image_path = os.path.join(height_charts_2_20_dir, f'height_chart_2_to_20_{idPat}.png')
        if os.path.exists(image_path):
            os.remove(image_path)
...
def generate_bmi_chart_2_to_20(request, idPat):
    try:
        ...
        # Guardar gráfico en el directorio de medios
        media_dir = settings.MEDIA_ROOT
        bmi_charts_2_20_dir = os.path.join(media_dir, 'bmi_charts_2_to_20')
        if not os.path.exists(bmi_charts_2_20_dir):
            os.makedirs(bmi_charts_2_20_dir)

        # Eliminar la imagen anterior si existe
        image_path = os.path.join(bmi_charts_2_20_dir, f'bmi_chart_2_to_20_{idPat}.png')
        if os.path.exists(image_path):
            os.remove(image_path)
...
def generate_growth_chart_girls_2_to_20(request, idPat):
    try:
        ...
        # Guardar gráfico en el directorio de medios
        media_dir = settings.MEDIA_ROOT
        growth_charts_girls_2_to_20_dir = os.path.join(media_dir, 'growth_charts_girls_2_to_20')
        if not os.path.exists(growth_charts_girls_2_to_20_dir):
            os.makedirs(growth_charts_girls_2_to_20_dir)

        # Eliminar la imagen anterior si existe
        image_path = os.path.join(growth_charts_girls_2_to_20_dir, f'growth_chart_girls_2_to_20_{idPat}.png')
        if os.path.exists(image_path):
            os.remove(image_path)
...
def generate_height_chart_girls_2_to_20(request, idPat):
    try:
        ...
        # Guardar gráfico en el directorio de medios
        media_dir = settings.MEDIA_ROOT
        height_charts_girls_2_to_20_dir = os.path.join(media_dir, 'height_charts_girls_2_to_20')
        if not os.path.exists(height_charts_girls_2_to_20_dir):
            os.makedirs(height_charts_girls_2_to_20_dir)

        # Eliminar la imagen anterior si existe
        image_path = os.path.join(height_charts_girls_2_to_20_dir, f'height_chart_girls_2_to_20_{idPat}.png')
        if os.path.exists(image_path):
            os.remove(image_path)
...
def viewrepCer(request, idobs):
    try:
        ...
        # Obtener la edad de la madre si está asociada
        if patbdd.mom:
            edad_madre = patbdd.mom.edad_mom
        else:
            edad_madre = "No disponible"
    ...
    return render(request, 'reports/reportcer.html', {
        'observaciones': obsbdd,
        'pacientes': patbdd,
        'diagnosticos': diabdd,
        'alergias': alergias,
        ...
        'edad_paciente': f"{edad_anios} años",
        'genero_paciente': genero_abreviado,
        'edad_madre': edad_madre  # Añadir la edad de la madre al contexto
    })
